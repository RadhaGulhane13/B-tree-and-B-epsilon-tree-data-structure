...
node* split_1(node *root) //split root into two nodes
{
...
node* delete_key_1(node* root, int key)
{
	if(root == NULL)
	{
		cout<<"key is not present"<<endl;
	...
	if(itr!=root->key_value.end())
	{
		...
		else
		{
			...
			if(right->second->key_value.size()-1 >= MAX_SIZE/2)
			{
				...
				right->second=delete_key_1(right->second,key_);
			}
			else
			{
				...
				root->key_value[key_]=value_;
				left->second=delete_key_1(left->second,key_);
			}

			//ifunderflow occur at left childe
			...
			if(root->key_value.size() == 0)//case where root key.size()==0 and contain one pivot 
			{
	...
	else //find appropriate child to find key
	{
		...
		itr2->second = delete_key_1(itr2->second,key);
			...
			if(itr2->second->key_value.size() < MAX_SIZE/2)
			{
				...
				if(itr2!= root->pivotmap.begin())
				{
					...
					if(prev_sibling->second->key_value.size()-1 >= MAX_SIZE/2)
					{
						...
						//assign last key of perv_sibling -key value 
...
node* insert(node* root,int key,int value)
{
	if(root==NULL)
	{
		...
		if(root->pivotmap.size() ==0)//if leaf node
		{
			...
			//find approriate child to insert//if equal => key already exists
			int count =0;
			auto itr2= root->pivotmap.begin();
			for(auto itr = root->key_value.begin();itr!=root->key_value.end();itr++)
			{
			...
			{//note:todo:merging if leaf node..i.e no child pointer how to split distribute also check whether this case can occur or not
				//cout<<"here:"<<temp->key_value.size()<<"  "<<temp->key_value.begin()->first;
				//add keyvalue to root keyvalue and pivot to roots pivot
				root->pivotmap.erase(itr2->first);
				root->key_value.insert(temp->key_value.begin(),temp->key_value.end());
				root->pivotmap.insert(temp->pivotmap.begin(),temp->pivotmap.end());
				temp=NULL;
				delete temp;
				//if kayvalur > max valur
				//split and return root
			...
			{
				itr2->second = temp;
				/*int key_ = temp->key_value.begin()->first;
				if(itr2->first > key_)
				{
				root->pivotmap[key_] =temp;
				root->pivotmap.erase(itr2->first);
				}
				else{
				itr2->second = temp;
...
int main()
{
	...
	node *root=NULL;
	//array element includes key, value , operation{1.insert 2.delete}
	...
	for(int i=0;i<2000;i++)
	{
		...
		{
		root= delete_key_1(root,arr[i][0]);
		}
		//display(root);  //display will display strcture of tree 
